...
#let project(
  ...
  let info = (
    [*Titel:*\ #{ title }], [*Tema*:\ #{ theme }], [*Projektperiode:*\ #{ projectperiod }], [*Projektgruppe:*\ #{ group-number }], if (authors.len() == 1) {
    ...
    [*Sidetal:* #str(calc.round(characters / 2400, digits: 1)) sider af #str(calc.trunc(characters)) anslag], [*Afleveringsdato:*\ #datetime.today().display("[day]/[month]-[year]")],
  )
  ...
  page(
    grid(
      columns: (1fr, 1fr), rows: (3fr, 7fr, 30pt), column-gutter: 10pt, image("UCN-hovedlogo-simple.svg", width: 80%), align(right + horizon)[
        #strong[Afdeling]\
        Professionshøjskolen UCN\
        #link("www.ucn.dk")
      ], grid(gutter: 16pt, ..info), [*Resumé*\ #box(width: 100%, stroke: .5pt, inset: 4pt, resume)],
    ),
  )
  ...
  set page( // standard page with header
    paper: "a4", margin: (top: 3.6cm, left: 2.2cm, right: 2.2cm, bottom: 3cm), header: context{
      grid(
        columns: (1fr, 1fr), align: (left, right), row-gutter: 0.5em, text(size: 11pt, context{ hydra(1, use-last: false, skip-starting: false) }), text(size: 11pt, number-type: "lining", {
  ...
  show heading.where(level: 1): it => {
    v(3.8 * body-size, weak: true) + block(it, height: 2 * body-size, sticky: true)
  }
